...
uint8_t mpu6500_read (uint8_t reg) {
    ...
    return rx [1];       // il dato è nel secondo byte
}
...
uint16_t get_angle_rotation (uint8_t reset_compass_flag) {
    ...
    float gz_corrected = reset_compass_flag ? 0.0f : (gz - biasZ);
    // integraton
